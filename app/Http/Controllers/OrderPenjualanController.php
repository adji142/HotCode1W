...
use App\Models\OrderPenjualanCopy;
use Carbon\Carbon;
use DB;
use PDF;
use EXCEL;
...
class OrderPenjualanController extends Controller
{
    protected $original_column = array(
      ...
      3 => "pj.orderpenjualan.tglso",
      4 => "pj.orderpenjualan.noso",
    ...
    public function index(Request $req)
    {
      ...
      $ppn = ($ppn) ? $ppn->value : 0;
      return view('transaksi.orderpenjualan.index', compact('ppn'));
    }

    public function getData(Request $req)
    {
      // gunakan permission dari indexnya aja
      if(!$req->user()->can('orderpenjualan.index')) {
      ...
      $filter_count = 0;
        ...
        13 => "pj.orderpenjualan.catatanpenjualan",
      );
      for ($i=1; $i < 10; $i++) {
      ...
      // $penjualan->where("pj.orderpenjualan.recordownerid",$req->session()->get('subcabang'))->where("pj.orderpenjualan.tglso",'>=',Carbon::parse($req->tglmulai))->where("pj.orderpenjualan.tglso",'<=',Carbon::parse($req->tglselesai));
      $penjualan->where("pj.orderpenjualan.recordownerid",$req->session()->get('subcabang'))->where("pj.orderpenjualan.tglpickinglist",'>=',Carbon::parse($req->tglmulai))->where("pj.orderpenjualan.tglpickinglist",'<=',Carbon::parse($req->tglselesai));
      ...
      if($empty_filter < 9){
        for ($i=1; $i < 10; $i++) {
          if ($req->custom_search[$i]['text']!='') {
            $index = $i;
            if($index == 2 || $index == 3 || $index == 4 || $index == 5 || $index == 6 || $index == 7 || $index == 9 ){
              if($req->custom_search[$i]['filter'] == '='){
                $penjualan->where(DB::raw($this->original_column[$index] . '::VARCHAR'),'ilike','%'.$req->custom_search[$i]['text'].'%');
              }else{
                $penjualan->where(DB::raw($this->original_column[$index] . '::VARCHAR'),'not ilike','%'.$req->custom_search[$i]['text'].'%');
              }
            }else{
      ...
      foreach ($penjualan->get() as $key => $jual) {
        ...
        if(strpos($jual->catatanpenjualan, "BATAL PICKING LIST") !== false)
        {
          $data[$key]['edit'] = 'Tidak bisa edit data. Picking List sudah dibatalkan. Hubungi Manager Anda.';
          $data[$key]['add'] = 'Tidak bisa tambah data. Picking List sudah dibatalkan. Hubungi Manager Anda.';
        }
        else
        {
        ...
        if(strpos($jual->catatanpenjualan, "BATAL PICKING LIST") !== false)
        {
          $data[$key]['batalpil'] = 'Tidak bisa batal record. Picking List sudah dibatalkan. Hubungi Manager Anda.';
        }
        else
        {
          if ($jual->checknota != null) {
        ...
        if(strpos($jual->catatanpenjualan, "BATAL PICKING LIST") !== false)
        {
          $data[$key]['cetakpil'] = 'Tidak bisa cetak dokumen. Picking List sudah dibatalkan. Hubungi Manager Anda.';
        }
        else
        {
          if ($jual->checkdetails == null) {
        ...
        if(strpos($jual->catatanpenjualan, "BATAL PICKING LIST") !== false)
        {
          $data[$key]['updatenoso'] = 'Tidak bisa Update No. SO. Picking List sudah dibatalkan. Hubungi Manager Anda.';
        }
        else
        {
          if ($jual->nota) {
        ...
        $data[$key]['copydooh'] = 'copydooh';
        if(strpos($jual->catatanpenjualan, "BATAL PICKING LIST") !== false)
        {
          $data[$key]['copydooh'] = 'Tidak bisa copy PiL ke OH. Picking List sudah dibatalkan. Hubungi Manager Anda.';
        }

        $data[$key]['copydodo'] = 'copydodo';
        if(strpos($jual->catatanpenjualan, "BATAL PICKING LIST") !== false)
        {
          $data[$key]['copydodo'] = 'Tidak bisa copy PiL ke PiL baru. Picking List sudah dibatalkan. Hubungi Manager Anda.';
        }
      }
    ...
    public function getDataDetail(Request $req)
    {
      ...
      if ($penjualan != null)
      {
        ...
        $penjualan_details = OrderPenjualanDetail::select([
            0 => "pj.orderpenjualandetail.id",
            1 => "pj.orderpenjualandetail.qtysoacc",
            2 => "mstr.stock.namabarang",
            3 => "mstr.stock.satuan",
            4 => "pj.orderpenjualandetail.hrgsatuanbrutto",
            5 => "pj.orderpenjualandetail.disc1",
            6 => "pj.orderpenjualandetail.disc2",
            7 => "pj.orderpenjualandetail.ppn",
          ])
          ->join("mstr.stock", "mstr.stock.id", "=", "pj.orderpenjualandetail.stockid")
          ->where("orderpenjualanid", $req->id);
        foreach ($req->custom_search as $key => $value) {
          if($key < 3)
          {
            if ($value["text"] != "")
            {
              if ($value["filter"] == "=")
                $penjualan_details->where("mstr.stock.".$columns[$key], "LIKE", "%".$value["text"]."%");
              elseif ($value["filter"] == "!=")
                $penjualan_details->where("mstr.stock.".$columns[$key], "NOT LIKE", "%".$value["text"]."%");
            }
          }
        }

        foreach ($penjualan_details->get() as $detail) {
          if ($penjualan->checknota != null) {
          ...
          if ($req->custom_search[3]["text"] != "")
          {
            switch ($req->custom_search[3]["filter"]) {
              case '<=':
                  if ((int)$detail->qtysoacc <= (int)($req->custom_search[3]["text"])) array_push($dontshow, 0); else array_push($dontshow, 1); break;
              case '<':
                  if ((int)$detail->qtysoacc < (int)($req->custom_search[3]["text"])) array_push($dontshow, 0); else array_push($dontshow, 1); break;
              case '=':
                  if ((int)$detail->qtysoacc == (int)($req->custom_search[3]["text"])) array_push($dontshow, 0); else array_push($dontshow, 1); break;
          ...
          if ($req->custom_search[4]["text"] != "")
          {
            switch ($req->custom_search[4]["filter"]) {
              case '<=':
                if ((int)$hrgnetto <= (int)($req->custom_search[4]["text"])) array_push($dontshow, 0); else array_push($dontshow, 1); break;
              case '<':
                if ((int)$hrgnetto < (int)($req->custom_search[4]["text"])) array_push($dontshow, 0); else array_push($dontshow, 1); break;
              case '=':
                if ((int)$hrgnetto == (int)($req->custom_search[4]["text"])) array_push($dontshow, 0); else array_push($dontshow, 1); break;
          ...
          if ($req->custom_search[5]["text"] != "")
          {
            switch ($req->custom_search[5]["filter"]) {
              case '<=':
                if ((int)($detail->qtysoacc * $hrgnetto) <= (int)($req->custom_search[5]["text"])) array_push($dontshow, 0); else array_push($dontshow, 1); break;
              case '<':
                if ((int)($detail->qtysoacc * $hrgnetto) < (int)($req->custom_search[5]["text"])) array_push($dontshow, 0); else array_push($dontshow, 1); break;
              case '=':
                if ((int)($detail->qtysoacc * $hrgnetto) == (int)($req->custom_search[5]["text"])) array_push($dontshow, 0); else array_push($dontshow, 1); break;
          ...
          if ($show == 0)
          {
            ...
            // ];
            $action = '';
            $action .= '<div class="btn btn-danger btn-xs skeyDel" data-toggle="tooltip" data-placement="bottom" title="Hapus - Del" onclick="deleteOrder(this,'.$detail->id.')" data-message="'.$delete.'" data-tipe="detail"><i class="fa fa-trash"></i></div>';
            $action .= '<div class="btn btn-danger btn-xs skeyF4" data-toggle="tooltip" data-placement="bottom" title="Batal Picking List -F4" onclick="batalPiLDetail(this,'.$detail->id.')" data-message="'.$batalpil.'" data-tipe="detail"><i class="fa fa-times"></i></div>';
            ...
            $temp = [
              ...
              'hrgnetto'   => number_format($hrgnetto,2,',','.'),
              'hrgtotal'   => number_format($detail->qtysoacc * $hrgnetto,0,',','.'),
              'delete'     => $delete,
              'batalpil'   => $batalpil,
              'ditolak'    => $ditolak,
            ];
    ...
    public function checkPILKadaluwarsa(Request $request){
      $params = $request->all();
      // $orderpenjualan = OrderPenjualan::find($request->id);
      $server_time_backup = Carbon::now();
      $server_time = Carbon::now();
      $tanggal_picking_list = Carbon::parse($params['id']['tglpickinglist']);
      $message = 'Picking List sudah kadaluwarsa. Silahkan COPY PIL atau HMA';
      
      if($server_time->format('Y') == $tanggal_picking_list->format('Y')){
        if($server_time->format('m') == $tanggal_picking_list->format('m')){
          if($server_time->format('d') == $tanggal_picking_list->format('d')){
            $message = 'skip';
          }
        }
      }

      // if($server_time->format('d') == $tanggal_picking_list->format('d') && $server_time->format('m') == $tanggal_picking_list->format('m') && $server_time->format('Y') == $tanggal_picking_list->format('Y')){
      //   return response()->json(['message' => $mesage]);
      // }else{
      //   return response()->json(['message' => 'Picking List sudah kadaluwarsa. Silahkan COPY PIL atau HMA']);
      // }
    ...
    public function getNextPickingListNo($recordownerid){
      ...
      if ($max_pj==null) {
        $next_no = '0000000001';
      }elseif (strlen($max_pj->nopickinglist)<10) {
        $next_no = '0000000001';
      }elseif ($max_pj->nopickinglist == '9999999999') {
        $next_no = '0000000001';
      }else {
        $next_no = substr('0000000000', 0, 10-strlen($max_pj->nopickinglist+1)).($max_pj->nopickinglist+1);
      }
      return $next_no;
    }
    ...
    public function tambah(Request $request)
    {
      if ($request->session()->get('subcabang')) {
        ...
        $ppn = ($ppn) ? $ppn->value : 0;
        return view('transaksi.orderpenjualan.form', compact('subcabanguser','expedisi','ppn'));
      }else {
    ...
    public function simpan(Request $request){
      ...
      $order->noso                = strtoupper($request->noso);
      $order->tglso               = Carbon::parse($request->tglso)->format("Y-m-d");
      ...
      $order->karyawanidsalesman  = $request->karyawanidsalesman;
      ...
      // Update Tokohakakses
      $tokohakakses = Tokohakakses::where('tokoid', $request->tokoid)
      ->where('recordownerid', $subcabang)
      ->update(['karyawanidsalesman' => $request->karyawanidsalesman]);
    ...
    public function kewenangan(Request $req)
    {
      ...
      {
        if(auth()->user()->can($req->permission) && $req->permission == 'orderpenjualan.hapus')
        {
    ...
    public function batalPiL(Request $req)
    {
      ...
      $opjd  = OrderPenjualanDetail::where('orderpenjualanid', $order->id)
               ->update(array("qtysoacc" => 0,'catatan'=>'BATAL PICKING LIST ' . 'TANGGAL ' . Carbon::now()->format("d-m-Y") . " OLEH " . auth()->user()->name));
      $order->catatanpenjualan = 'BATAL PICKING LIST ' . 'TANGGAL ' . Carbon::now()->format("d-m-Y") . " OLEH " . auth()->user()->name;
      $order->save();
      return response()->json(['success'=> true]);
    }
    ...
    public function batalPiLDetail(Request $req)
    {
      $detail = OrderPenjualanDetail::find($req->id);
      $detail ->qtysoacc = 0;
      $detail ->catatan  = 'BATAL PICKING LIST ' . 'TANGGAL ' . Carbon::now()->format("d-m-Y") . " OLEH " . auth()->user()->name;
      $detail ->save();
      return response()->json(['success'=> true]);
    }
    ...
    public function cetakPicking(Request $req,$id)
    {
        ...
        $pdf   = PDF::loadView('transaksi.orderpenjualan.picking_2',['user'=>$req->user(),'cetak'=>$cetak,'details'=>$details],[],$config);
    ...
    public function cetakPicking_test(Request $req,$id)
    {
        ...
        $pdf   = PDF::loadView('transaksi.orderpenjualan.picking',['user'=>$req->user(),'cetak'=>$cetak,'details'=>$details],[],$config);
        return $pdf->stream('CetakPickingList-'.$req->id.'.pdf');
    }
    ...
    public function riwayatJual(Request $req,$toko)
    {
      ...
      $rjual = Toko::select([
          ...
          1 => "mstr.toko.alamat",
          2 => "mstr.toko.kota",
          3 => "mstr.toko.propinsi",
          4 => "mstr.toko.customwilayah",
          5 => DB::raw("to_char(mstr.toko.lastupdatedon, 'YYYY-MM-DD') as tgllastupdate"),
        ])
      ...
      $user = $req->user();
      $filename = "Riwayat Jual_" . $rjual[0]->namatoko . "_" . Carbon::now()->format("dmYHis");
      Excel::create($filename, function($excel) use ($rjual,$data,$totalharga,$user) {
        ...
        $excel->sheet('RiwayatJual', function($sheet) use ($rjual,$data,$totalharga,$user) {
            $sheet->loadView('transaksi.orderpenjualan.riwayat',array(
              'rjual' =>$rjual[0],
              'notajual'=>$data,
      ...
      return redirect(asset('storage/excel/exports/' . $filename .'.xls'));
    }
    ...
    public function cekDodo(Request $req){
      $success = false;
      $chekPIL = OrderPenjualanCopy::selectRaw("pj.orderpenjualancopy.id, pj.orderpenjualancopy.orderpenjualanidlama")
        ->where("pj.orderpenjualancopy.orderpenjualanidlama", $req->id)
        ->get();
    ...
    public function insertDodo(Request $req)
    {
      ...
      $orderpenjualannew = OrderPenjualan::create([
        ...
        'catatanpenjualan'    => strtoupper($orderpenjualan->catatanpenjualan) . " PiL Copy Dari PiL " . $orderpenjualan->nopickinglist . " tanggal " . Carbon::parse($orderpenjualan->tglpickinglist)->format("d-m-Y") ,
      ...
      $checkSumber = OrderPenjualanCopy::selectRaw("pj.orderpenjualancopy.orderpenjualanidsumber")
        ->where("pj.orderpenjualancopy.orderpenjualanidbaru",$req->id);
    
      if ($checkSumber->count() > 0){
        $row = $checkSumber->first();
        $sumberid = $row->orderpenjualanidsumber;
      }

      $orderpenjualancopy = OrderPenjualanCopy::create([
        'orderpenjualanidsumber' => $sumberid,
        'orderpenjualanidlama'  =>  $req->id,
        'orderpenjualanidbaru'  =>  $orderpenjualannew->id,
        'createdby'  =>  strtoupper(auth()->user()->username),
    ...
    public function getAcc11(Request $req)
    {
      ...
      try {
        ...
        // check minimal data
        $allow = array("RowID", "ID", "LastUpdatedBy", "Details");
        foreach($data as $k => $v) {
        ...
        if(count($dat) <= 0) {
          ...
          $dat->lastupdatedby    = $data["LastUpdatedBy"];
    ...
    public function cekBarangopj(Request $req)
    {
      if ($req->stockid && $req->tokoid) {
        $opj    = OrderPenjualan::find($req->id);
        $cekopj = OrderPenjualan::select('tglpickinglist','nopickinglist','namatoko','namabarang')
        ->join('pj.orderpenjualandetail','pj.orderpenjualandetail.orderpenjualanid','=','pj.orderpenjualan.id')
        ->leftJoin('mstr.toko','pj.orderpenjualan.tokoid','=','mstr.toko.id')
        ->leftJoin('mstr.stock','pj.orderpenjualandetail.stockid','=','mstr.stock.id')
        ->where('tglpickinglist','=',Carbon::parse($opj->tglpickinglist))
        ->where('tokoid','=',$opj->tokoid)
        ->where('stockid','=',$req->stockid)
        ->first();

        if(count($cekopj) > 0) return response()->json($cekopj);
      }
