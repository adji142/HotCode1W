...
<div class="">
	<div class="row">
		<div class="x_panel">
			<div class="x_title">
				<h2>Laporan Stock Akhir Periode</h2>
						...
						<form name="frmKSLap" class="form-label-left" style="margin-bottom: 10px;">
							...
							<div xopt="all|group" class="form-group" style="display: none;">
								...
								<div>
									...
									<div class="radio-inline">
										<label><input type="radio" name="status" value="all" selected>SEMUA</label>
									</div>
								</div>
							</div>
	...
	$(document).ready(function () {
		...
		var frm = $("form[name=frmKSLap]", document).submit(function (e) {
			...
			var dt = {
				...
				'subcabang' : $("[name=subcid]", this).val().trim()
			};

			switch (md) {
				case "all":
					dt['status'] = $("[name=status]:checked", this).val() == "active" ? 1 : 0;
					break;
				case "specific":
			...
			if (allow) {
				...
				var url = '{{ route("laporan.stock.preview", ["dname"=>"stockakhirperiode"]) }}?' + dtp;
				window.open(url);
			}
		});
